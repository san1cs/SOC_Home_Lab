...
* **Action & Objective:** Performed a Nmap 'Port' scan to see the active services running on ports 80 (HTTP), to demonstrate how the service is visible during the port scan without firewall rules.
...
* **Result:** The firewall successfully blocked the outgoing http connection.
...
### 4.1.Understanding Network Traffic

* **Inbound Traffic:** Requests coming from the outside (e.g., someone trying to access your web server).

* **Outbound Traffic:** Requests going from your machine to the world (e.g., you downloading a software update).

### 4.2.The Role of Ports & Firewall

* A ports are the specific "doors" on the computer that allow different types of information to pass through.

* A firewall acts as the lock on those doors, only opens to the authorized so the computer stays safe from unwanted visitors.
